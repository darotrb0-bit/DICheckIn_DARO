...
// --- Function សម្រាប់ទាញទិន្នន័យច្បាប់ (Leave) ---
async function fetchAllLeaveForMonth(employeeId) {
  ...
  const outCollectionPath = "/artifacts/default-app-id/public/data/out_requests";
      ...
      if (isMultiDay) {
        ...
        if (startDate >= startMonthDate && startDate <= endMonthDate) {
          ...
          if (durationStr === "មួយថ្ងៃ" || durationStr === "មួយយប់") {
            allLeaveRecords.push({ date: dateStr, formattedDate: formatted, checkIn: leaveLabel, checkOut: leaveLabel });
          } else if (durationStr === "មួយព្រឹក") {
            allLeaveRecords.push({ date: dateStr, formattedDate: formatted, checkIn: leaveLabel, checkOut: null });
          } else if (durationStr === "មួយរសៀល") {
            allLeaveRecords.push({ date: dateStr, formattedDate: formatted, checkIn: null, checkOut: leaveLabel });
          }
        }
      }
    });
  } catch (e) {
    console.error("Error fetching 'leave_requests'", e);
  }
  ...
  try {
    ...
    console.error("Error fetching 'out_requests'", e);
  }
...
async function loadAIModels() {
  ...
  try {
    await faceapi.nets.tinyFaceDetector.loadFromUri(MODEL_URL, { useDiskCache: true });
    await faceapi.nets.faceLandmark68Net.loadFromUri(MODEL_URL, { useDiskCache: true });
    await faceapi.nets.faceRecognitionNet.loadFromUri(MODEL_URL, { useDiskCache: true });
...
async function checkLeaveStatus(employeeId, checkType) {
  ...
  try {
    ...
    if (leaveType === "មួយថ្ងៃ") return { blocked: true, reason: `ច្បាប់ចេញក្រៅមួយថ្ងៃ (${reason})` };
    if (leaveType === "មួយព្រឹក" && checkType === "checkIn") return { blocked: true, reason: `ច្បាប់ចេញក្រៅមួយព្រឹក (${reason})` };
    if (leaveType === "មួយរសៀល" && checkType === "checkOut") return { blocked: true, reason: `ច្បាប់ចេញក្រៅមួយរសៀល (${reason})` };
    return null;
  } catch (error) {
    console.error("Error checking [out_requests]", error);
...
async function checkFullLeaveStatus(employeeId, checkType) {
  ...
  const q = query(collection(dbLeave, leaveCollectionPath), where("userId", "==", employeeId), where("status", "==", "approved"));
      ...
      if (isMultiDay) {
        ...
        if (todayTimestamp >= startTimestamp && todayTimestamp <= endTimestamp) {
          ...
          if (isHalfDay && todayTimestamp === endTimestamp) {
            if (checkType === "checkIn") return { blocked: true, reason: `ច្បាប់ ${durationStr} (ព្រឹក) (${reason})` };
            else continue;
          }
        ...
        if (startDateStr === todayString_DD_Mon_YYYY) {
          if (durationStr === "មួយថ្ងៃ" || durationStr === "មួយយប់") return { blocked: true, reason: `ច្បាប់ ${durationStr} (${reason})` };
          if (durationStr === "មួយព្រឹក" && checkType === "checkIn") return { blocked: true, reason: `ច្បាប់មួយព្រឹក (${reason})` };
          if (durationStr === "មួយរសៀល" && checkType === "checkOut") return { blocked: true, reason: `ច្បាប់មួយរសៀល (${reason})` };
        }
      }
    }
    return null;
  } catch (error) {
    console.error("Error checking [leave_requests]", error);
    return { blocked: true, reason: "Error checking leave status." };
  }
}
...
async function startFaceScan(action) {
  ...
  if (!currentUserFaceMatcher) {
    showMessage("បញ្ហា", "មិនអាចស្កេនមុខបានទេ។ អាចមកពីមិនមានរូបថត Profile។", true);
    return;
  }
...
async function fetchGoogleSheetData() {
  ...
  try {
    ...
    if (!jsonText || !jsonText[1]) throw new Error("Invalid Gviz response");
    ...
    allEmployees = data.table.rows
      ...
      .filter((emp) => emp !== null && emp.group !== "ការងារក្រៅ" && emp.group !== "បុគ្គលិក");
    ...
    if (savedEmployeeId) {
      ...
      if (savedEmployee) selectUser(savedEmployee);
      else {
    ...
    showMessage("បញ្ហាទាញទិន្នន័យ", `មិនអាចទាញទិន្នន័យពី Google Sheet បានទេ: ${error.message}`, true);
  }
}
...
function renderEmployeeList(employees) {
  ...
  employees.forEach((emp) => {
    ...
    card.innerHTML = `
      <img src="${emp.photoUrl || "https://placehold.co/48x48/e2e8f0/64748b?text=No+Img"}" class="w-12 h-12 rounded-full object-cover border-2 border-gray-100 mr-3" onerror="this.src='https://placehold.co/48x48/e2e8f0/64748b?text=Error'">
      <div><h3 class="text-md font-semibold text-gray-800">${emp.name}</h3><p class="text-sm text-gray-500">ID: ${emp.id} | ក្រុម: ${emp.group}</p></div>
    `;
...
async function selectUser(employee) {
  ...
  try {
    ...
    await setDoc(sessionDocRef, { deviceId: currentDeviceId, timestamp: new Date().toISOString(), employeeName: employee.name });
  } catch (e) {
  ...
  const simpleDataPath = `attendance/${currentUser.id}/records`;
...
function logout() {
  ...
  if (leaveCollectionListener) { leaveCollectionListener(); leaveCollectionListener = null; }
  if (outCollectionListener) { outCollectionListener(); outCollectionListener = null; }
...
function startSessionListener(employeeId) {
  ...
  sessionListener = onSnapshot(sessionDocRef, (docSnap) => {
    ...
    if (localDeviceId && sessionData.deviceId !== localDeviceId) {
...
function forceLogout(message) {
  logout();
  showMessage("បានចាកចេញដោយស្វ័យប្រវត្តិ", message, true);
  currentConfirmCallback = () => { hideMessage(); changeView("employeeListView"); };
}
...
function startLeaveListeners() {
  ...
  const qLeave = query(collection(dbLeave, leaveCollectionPath), where("userId", "==", employeeId));
  leaveCollectionListener = onSnapshot(qLeave, () => reFetchAllLeave());

  const qOut = query(collection(dbLeave, outCollectionPath), where("userId", "==", employeeId));
...
// --- UI FIX: ប្រើប្រាស់ Grid Layout ដើម្បីតម្រឹមអក្សរឱ្យស្អាត (Monthly History) ---
function renderMonthlyHistory() {
  ...
  currentMonthRecords.forEach((record) => {
    ...
    // បង្កើត HTML សម្រាប់ម៉ោង (មិនប្រើ span ស្មុគស្មាញ ដើម្បីកុំឱ្យបែកជួរ)
    let checkInDisplay = record.checkIn 
      ? (record.checkIn.match(/AM|PM/) 
          ? `<span class="text-green-600 font-bold">${record.checkIn}</span>` 
          : `<span class="text-blue-600 font-bold">${record.checkIn}</span>`) 
      : (isToday ? "---" : '<span class="text-red-500 font-medium">អវត្តមាន</span>');

    let checkOutDisplay = record.checkOut 
      ? (record.checkOut.match(/AM|PM/) 
          ? `<span class="text-red-600 font-bold">${record.checkOut}</span>` 
          : `<span class="text-blue-600 font-bold">${record.checkOut}</span>`) 
      : (isToday ? '<span class="text-gray-400">មិនទាន់ចេញ</span>' : '<span class="text-red-500 font-medium">អវត្តមាន</span>');

    const card = document.createElement("div");
    card.className = "bg-white p-4 rounded-xl shadow-sm border border-gray-100";
    
    // ប្រើ CSS Grid ដើម្បីតម្រឹម (ឆ្វេង: Label, ស្ដាំ: Value)
    card.innerHTML = `
      <div class="flex justify-between items-center mb-3">
         <span class="font-bold text-gray-800 text-sm">${formattedDate}</span>
         ${isToday ? '<span class="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-medium">ថ្ងៃនេះ</span>' : ''}
      </div>
      <div class="grid grid-cols-[50px_1fr] gap-y-2 text-sm items-center">
        <span class="text-gray-500 font-medium">ចូល:</span>
        <div class="text-right">${checkInDisplay}</div>
        
        <span class="text-gray-500 font-medium">ចេញ:</span>
        <div class="text-right">${checkOutDisplay}</div>
      </div>
    `;
...
// --- UI FIX: ប្រើប្រាស់ Grid Layout ដើម្បីតម្រឹមអក្សរឱ្យស្អាត (Today History) ---
function renderTodayHistory() {
  ...
  let checkInDisplay = todayRecord.checkIn 
    ? (todayRecord.checkIn.match(/AM|PM/) 
        ? `<span class="text-green-600 font-bold">${todayRecord.checkIn}</span>` 
        : `<span class="text-blue-600 font-bold">${todayRecord.checkIn}</span>`) 
    : "---";

  let checkOutDisplay = todayRecord.checkOut 
    ? (todayRecord.checkOut.match(/AM|PM/) 
        ? `<span class="text-red-600 font-bold">${todayRecord.checkOut}</span>` 
        : `<span class="text-blue-600 font-bold">${todayRecord.checkOut}</span>`) 
    : '<span class="text-gray-400">មិនទាន់ចេញ</span>';
  
  const card = document.createElement("div");
  card.className = "bg-blue-50 p-5 rounded-xl shadow-sm border border-blue-100";
  
  // ប្រើ CSS Grid ដូចគ្នា
  card.innerHTML = `
    <div class="flex justify-between items-center mb-3">
       <span class="font-bold text-blue-900 text-base">${formattedDate}</span>
    </div>
    <div class="grid grid-cols-[50px_1fr] gap-y-3 text-sm items-center">
      <span class="text-blue-700 font-medium">ចូល:</span>
      <div class="text-right text-base">${checkInDisplay}</div>
      
      <span class="text-blue-700 font-medium">ចេញ:</span>
      <div class="text-right text-base">${checkOutDisplay}</div>
    </div>
  `;
  container.appendChild(card);
}
...
async function updateButtonState() {
  ...
  // តម្រូវ Logic: បិទបើកប៊ូតុង និងបង្ហាញសារ
  ...
  if (todayData && todayData.checkIn) {
    ...
    if (todayData.checkIn.match(/AM|PM/)) {
  ...
  if (todayData && todayData.checkIn && !todayData.checkOut) {
    ...
    // បើអាច Check-out បាន តែមិនទាន់ដល់ម៉ោង (ឧ. បៃតង)
    if (todayData.checkIn.match(/AM|PM/) && !checkOutDisabled) {
       // រក្សាសារ "បាន Check-in..."
    }
  } else if (todayData && todayData.checkOut) {
    checkOutDisabled = true;
    if (todayData.checkOut.match(/AM|PM/)) {
  ...
  attendanceStatus.className = `text-center text-sm pb-4 px-6 h-5 ${statusClass} font-semibold`;
}

// --- Check-in Function (NO LOCATION) ---
...
// --- Check-out Function (NO LOCATION) ---
...
searchInput.addEventListener("input", (e) => {
  ...
  const filteredEmployees = allEmployees.filter((emp) => emp.name.toLowerCase().includes(searchTerm) || emp.id.toLowerCase().includes(searchTerm));
...
logoutButton.addEventListener("click", () => {
  showConfirmation("ចាកចេញ", "តើអ្នកប្រាកដជាចង់ចាកចេញមែនទេ? គណនីរបស់អ្នកនឹងមិនត្រូវបានចងចាំទៀតទេ។", "ចាកចេញ", () => { logout(); hideMessage(); });
});
exitAppButton.addEventListener("click", () => {
  showConfirmation("បិទកម្មវិធី", "តើអ្នកប្រាកដជាចង់បិទកម្មវិធីមែនទេ?", "បិទកម្មវិធី", () => { window.close(); hideMessage(); });
});
...
modalConfirmButton.addEventListener("click", () => { if (currentConfirmCallback) currentConfirmCallback(); else hideMessage(); });
...
navHistoryButton.addEventListener("click", () => { changeView("historyView"); navHomeButton.classList.remove("active-nav"); navHistoryButton.classList.add("active-nav"); });
